...
namespace MaHoaSoHoc
{
	public abstract class Crypter
	{
		...
		public abstract byte[] Encipher(byte[] idata);
		public abstract byte[] Decipher(byte[] idata);
	}
	/// <summary>
	/// Class that provides blowfish encryption.
	/// </summary>
	public class MaHoa: Crypter
	{
		const int	N = 16;
		const int	MAX_KEY_BYTES = 56;
		...
		//---
		public MaHoa() {}
		public MaHoa(MainForm frm)
		{
			parent = frm;
		}
		public MaHoa(byte[] key)
		{
			InitBoxes(key);
		}
		/// <summary>
		...
		private UInt32 F(UInt32 x)
		{
			...
			//.. y = (S[0,a] + S[1, b] mod 2**32) XOR S[2, c] + S[3, d] mod 2**32.
			y += (UInt64)S[0,a] + (UInt64)S[1,b];
			y ^= S[2,c];
			y += S[3,d];
			y &= 0xFFFFFFFF;
		...
		/// <param name="xl"></param>
		/// <param name="xr"></param>
		private void Encipher(ref UInt32 xl, ref UInt32 xr)
		{
			UInt32	Xl;
			UInt32	Xr;
			UInt32	temp;
			...
			//.. Perform Feistel cycle 16 times.
			for (i = 0; i < N; ++i)
			{
				Xl = Xl ^ P[i];
				Xr = F(Xl) ^ Xr;
			...
			//.. exchange left and right (don't exchange left and right
			// at 17th performance).
			temp = Xl;
			Xl = Xr;
			Xr = temp;
		...
		public override byte[] Encipher(byte[] idata)
		{
			//if (idata.Length < 8) return idata;
			
			int length = (idata.Length/8)*8;
			byte[] odata = idata.Clone() as byte[];
			
			UInt32 xl=0, xr=0;
			for (int i=0; i<length; i+=8)
			{
				xl = ((UInt32)odata[i] << 24) | ((UInt32)odata[i+1] << 16) | ((UInt32)odata[i+2] << 8) | (UInt32)odata[i+3];
				xr = ((UInt32)odata[i+4] << 24) | ((UInt32)odata[i+5] << 16) | ((UInt32)odata[i+6] << 8) | (UInt32)odata[i+7];
				Encipher(ref xl, ref xr);
				odata[i] = (byte)((xl >> 24) & 0xFF);
				odata[i+1] = (byte)((xl >> 16) & 0xFF);
				odata[i+2] = (byte)((xl >> 8) & 0xFF);
				odata[i+3] = (byte)(xl & 0xFF);
				odata[i+4] = (byte)((xr >> 24) & 0xFF);
				odata[i+5] = (byte)((xr >> 16) & 0xFF);
				odata[i+6] = (byte)((xr >> 8) & 0xFF);
				odata[i+7] = (byte)(xr & 0xFF);
			}
		...
		/// <param name="data"></param>
		/// <param name="length"></param>
		public override byte[] Decipher(byte[] idata)
		{
			//if (idata.Length < 8) return idata;
		...
		//---
		public bool Encipher(string fin, string fout)
		{
			...
			try
			{
				...
				rin = new BinaryReader(fsin);
				wout = new BinaryWriter(fsout);
			...
			{
				MessageBox.Show("Lỗi: " + ioe.Message, "Exception!");
				//throw ioe;
			}
			finally
			{
		...
		/// <returns></returns>
		public bool Decipher(string fin, string fout)
		{
			FileStream fsin = null;
			FileStream fsout = null;
			BinaryReader rin = null;
		...
		/// <summary>
		/// Information about the encipher algorithm.
		/// </summary>
		public static string Message = "Blowfish is a symmetric encryption algorithm, "
			+ "meaning that it uses the same secret key to both encrypt and decrypt "
			+ "messages. Blowfish is also a block cipher, meaning that it divides a "
			+ "message up into fixed length blocks during encryption and decryption. "
